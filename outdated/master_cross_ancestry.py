...
# This script should execute 'cross_ancestry analysis'
# Inputs:
#   Settings:   YAML file
#   Seeds:      List of seeds 
...
# Run the script with each seed
# 1. Iterate over all seeds
# 2. Modify the output directory of 'YAML_FILE' for each seed
# 3. Create temporary directory to save setting files
# 4. Create qsub output direcotry
# 5. Submit to sun_grid_engine with qsub with modified seed and output_path

# Run 'main_cross_ancestry' analysis 
...
analysis_name = f"{train_ancestry}_to_{infer_ancestry}"
...
for seed in SEEDS:
    ...
    # Create 'tmp' directory on vscratch
...
job_name = f"{tag}_{comparison}_combine_runs"

# qsub log files
output_log = os.path.join(qsub_directory, f"{job_name}_output.log")
error_log = os.path.join(qsub_directory, f"{job_name}_error.log")
